...
__global__ void updateSourceSink(double *smp, double *smn, double *smvRp, double *smvRn, double *smvZp, double *smvZn,
  ...
  double massp = atomicMass*AMU;
  double massn = atomicMass*AMU;
  ...
  smvRp[gridPos] = mp[gridPos]*((q*Er[gridPos]/massp)-(getGamma()*vpr))+np*kBoltz*Tp/massp;//Radial positive momentum source
  smvRn[gridPos] = mn[gridPos]*((-q*Er[gridPos]/massn)-(getGamma()*vnr))+nn*kBoltz*Tn/massn;//Radial negative momentum source
  smvZp[gridPos] = mp[gridPos]*((q*Ez[gridPos]/massp)-(getGamma()*vpz));//Axial positive momentum source
  smvZn[gridPos] = mn[gridPos]*((-q*Ez[gridPos]/massn)-(getGamma()*vnz));//Axial negative momentum source
}
