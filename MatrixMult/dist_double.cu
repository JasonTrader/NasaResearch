...
int main(){
  ...
  mat_a.elements = (double *)temp_a;
  mat_b.elements = (double *)temp_b;
  mat_c.elements = (double *)temp_c;
  ...
  for(i = 0; i<arow; i++){
    for(j = 0; j<acol; j++){
      mat_a.elements[i*acol+j] = (double)(rand()%1000000)/17;
    }
  }

  for(i = 0; i<brow; i++){
    for(j = 0; j<bcol; j++){
      mat_b.elements[i*bcol+j] = (double)(rand()%1000000)/23;
    }
  }
  mat_a.height = arow;
  mat_a.width = acol;
  ...
  for(i = 0; i<crow; i++){
    for(j = 0; j<ccol; j++){
      fprintf(f, "%lf ", mat_c.elements[i*ccol+j]);
    }
    fprintf(f, "\n");
  }
  fclose(f);
